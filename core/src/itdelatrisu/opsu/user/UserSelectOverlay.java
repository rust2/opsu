...
public class UserSelectOverlay extends AbstractComponent {
    ...
    public UserSelectOverlay(GameContainer container, UserSelectOverlayListener listener)
    {
        ...
        //region edit: kww
        this.width = UserButton.getWidth() + UserButton.getPadding() * 10;

        //endregion
        this.height = containerHeight;
        this.x = (containerWidth - width) / 2f;
    ...
    private void renderUserSelect(Graphics g, float alpha)
    {
        ...
        Fonts.XLARGE.drawString(
                x + (width - Fonts.XLARGE.getWidth(title)) / 2f,
    ...
    private void renderUserCreate(Graphics g, float alpha)
    {
        ...
        Fonts.MEDIUM.drawString(x + (width - Fonts.MEDIUM.getWidth(caption)) / 2f, cy, caption, COLOR_WHITE);
        ...
        Fonts.MEDIUMBOLD.drawString(x + (width - Fonts.MEDIUMBOLD.getWidth(nameHeader)) / 2f, cy, nameHeader, COLOR_WHITE);
    ...
    {
        Fonts.MEDIUMBOLD.drawString(x + (width - Fonts.MEDIUMBOLD.getWidth(header)) / 2f, cy, header, COLOR_WHITE);
        ...
        for (int i = 0; i < userIcons.length; i += maxPerLine)
        {
            ...
            for (int j = 0; j < n; j++)
            {
                ...
                button.setX(cx + iconSize / 2f);
                button.setY(cy + iconSize / 2f);
                if (cy < height)
                {
    ...
    { /* does not currently use the "focus" concept */ }
}
