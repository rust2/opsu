		...
		*/
	{
		...
		if (beatmap.breaks != null && breakIndex < beatmap.breaks.size() && breakTime > 0) {
				...
				trackPosition - breakTime < 5000)
			{
				...
				int msUntilBreakEnd = endTime - trackPosition;

				// It blinks 7 times
				if(UselessUtils.Maths.inRange(1164, msUntilBreakEnd, 1261) ||
						UselessUtils.Maths.inRange(970, msUntilBreakEnd, 1067) ||
						UselessUtils.Maths.inRange(776, msUntilBreakEnd, 873) ||
						UselessUtils.Maths.inRange(582, msUntilBreakEnd, 679) ||
						UselessUtils.Maths.inRange(388, msUntilBreakEnd, 485) ||
						UselessUtils.Maths.inRange(194, msUntilBreakEnd, 291) ||
						UselessUtils.Maths.inRange(0, msUntilBreakEnd, 97)) {
					Image arrow = GameImage.WARNINGARROW.getImage(); // todo: cache this image (don't retrieve it every frame)
		...
		if (pauseTime > -1 && pausedMousePosition != null) {
			...
			*/
			//endregion
			int circleDiameter = GameImage.HITCIRCLE.getImage().getWidth();
			Image cursorCircle = GameImage.CURSOR.getDefaultImage().getScaledCopy(circleDiameter, circleDiameter);
			//cursorCircle.setAlpha(1.0f);
			Color255 orange = new Color255(255, 64, 0); // 0xff6400 // orange
			cursorCircle.drawCentered(pausedMousePosition.x, pausedMousePosition.y, orange.fake());
			Image cursorCirclePulse = cursorCircle.getScaledCopy(1f + pausePulse);
			cursorCirclePulse.setAlpha(1f - pausePulse);
			cursorCirclePulse.drawCentered(pausedMousePosition.x, pausedMousePosition.y);
		}
...
//		container.setMouseGrabbed(true);
	...
	@Override
	public void leave(StateBasedGame game)
			throws SlickException {
